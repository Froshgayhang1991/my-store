// Scroll to product card when ticker item is clicked
function scrollToProduct(code) {
  var newSection = document.querySelector('.category-section[data-category="NEW_PRODUCTS"]');
  if (newSection) {
  ...
  var card = document.querySelector('.product[data-code="' + code + '"]');
  if (card) {
    setTimeout(function() {
      card.scrollIntoView({ behavior: 'smooth', block: 'center' });
      card.classList.add('ticker-highlight');
      setTimeout(function() { card.classList.remove('ticker-highlight'); }, 3600);
    }, 150);
  }
}
...
function validateAndFindAgent(input) {
  ...
  for (let i = 0; i < options.length; i++) {
    ...
    if (option.value && option.dataset.lastfour === code) {
      matchingAgents.push({ element: option, value: option.value, text: option.text, mobile: option.dataset.mobile, address: option.dataset.address });
    }
  }
...
function decreaseQuantity(event, btn) {
  ...
  if (currentValue > 0) { input.value = Math.max(0, currentValue - 1); updateCart(); }
}
...
function increaseInvoiceQuantity(inputId) {
  ...
  invoiceInput.value = (parseFloat(invoiceInput.value) || 0) + 1;
  mainInput.value = invoiceInput.value;
  updateInvoiceTotals(); updateCart();
}
...
function decreaseInvoiceQuantity(inputId) {
  ...
  if (currentValue > 0) {
    ...
    invoiceInput.value = newValue; mainInput.value = newValue;
    updateInvoiceTotals(); updateCart();
  }
}
...
function addHalfInvoiceQuantity(inputId) {
  const invoiceInput = document.getElementById('invoice_qty_' + inputId);
  const mainInput = document.getElementById('qty_' + inputId);
  invoiceInput.value = (parseFloat(invoiceInput.value) || 0) + 0.5;
  mainInput.value = invoiceInput.value;
  updateInvoiceTotals(); updateCart();
}
...
function updateCart() {
  ...
  document.getElementById('cartSection').style.display = totalQuantity > 0 ? 'flex' : 'none';
}
...
function showInvoice() {
  ...
  if (!agentSelect || !agentSelect.value) {
    ...
    if (document.getElementById('agentCode')) document.getElementById('agentCode').focus();
    return;
  }
  const products = [];
  ...
  document.querySelectorAll('.quantity').forEach(input => {
    ...
    if (qty > 0) {
      const price = parseFloat(input.dataset.price) || 0;
      products.push({ name: input.dataset.name, code: input.dataset.code, price: price, qty: qty, total: qty * price, inputId: input.id.replace('qty_', '') });
      totalPrice += qty * price; totalQty += qty;
    }
  });
  if (products.length === 0) { alert(GetLabel('cart_alert_msg')); return; }
  const now = new Date();
  ...
  products.forEach((product, index) => {
    ...
    itemDiv.innerHTML = `
      ...
      <div class='invoice-item-controls'>
        ...
        <div class='invoice-item-total'><strong>${product.total.toLocaleString()} ${GetLabel('dinar')}</strong></div>
      </div>`;
...
function updateInvoiceTotals() {
  ...
  document.querySelectorAll('.invoice-quantity').forEach(input => {
    ...
    if (itemTotalElement) itemTotalElement.textContent = itemTotal.toLocaleString() + ' ' + GetLabel('dinar');
    if (qty > 0) { totalPrice += itemTotal; totalQty += qty; }
  });
...
window.onclick = function(event) {
  ...
  if (event.target === modal) closeInvoice();
};
...
function sendToWhatsApp() {
  ...
  document.querySelectorAll('.quantity').forEach(input => {
    ...
    if (qty > 0) {
      const price = parseFloat(input.dataset.price) || 0;
      products.push({ name: input.dataset.name, code: input.dataset.code, price: price, qty: qty, total: qty * price });
      totalPrice += qty * price; totalQty += qty;
    }
  });
  if (products.length === 0) { alert('Please select at least one product'); return; }
  const agentSelect = document.getElementById('agentSelect');
  if (!agentSelect || !agentSelect.value) { alert('Please select an agent first'); document.getElementById('agentCode').focus(); return; }
  const selectedOption = agentSelect.options[agentSelect.selectedIndex];
  ...
  message += date + ' - ' + time + '%0A%0A';
  message += GetLabel('agent_info') + '%0A';
  message += GetLabel('name_label') + ': ' + agentName + '%0A';
  message += GetLabel('mobile_label') + ': ' + agentMobile + '%0A';
  message += GetLabel('address_label') + ': ' + agentAddress + '%0A%0A';
  message += GetLabel('product_list') + '%0A';
  products.forEach((product, index) => {
    message += '' + (index + 1) + ' - ' + product.name + ' - ' + product.code + '%0A';
    message += '   ' + GetLabel('quantity_label') + ': (' + product.qty + ') - ' + GetLabel('price_label') + ': ' + product.price.toLocaleString() + ' - ' + GetLabel('total_label') + ': ' + product.total.toLocaleString() + '%0A';
  });
  message += '%0A' + GetLabel('total_quantity') + ': ' + totalQty + '%0A';
  message += GetLabel('total_price') + ': ' + totalPrice.toLocaleString() + ' ' + GetLabel('dinar') + '%0A%0A';
  ...
  setTimeout(() => {
    if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
      ...
      link.href = whatsappURL; link.target = '_blank'; link.style.display = 'none';
...
window.addEventListener('scroll', function() {
  ...
  const isCategorySelected = Array.from(document.querySelectorAll('.category-section')).some(s => !s.classList.contains('hidden'));
  backBtn.style.display = (isCategorySelected && window.scrollY > 300) ? 'flex' : 'none';
});

function scrollToCategories() {
  const categoryButtons = document.getElementById('categoryButtons');
  if (categoryButtons) categoryButtons.scrollIntoView({ behavior: 'smooth', block: 'start' });
}
